module.exports = {
    ...
    execute(message, args, Discord, client, prefix, colors, command){
        if (!args[0]){
            ...
            const util = fs.readdirSync(`src/commands/util`).toString().split(`.js,`).join(`, `).slice(0, -3);
            const info = fs.readdirSync(`src/commands/info`).toString().split(`.js,`).join(`, `).slice(0, -3);
            const fun = fs.readdirSync(`src/commands/fun`).toString().split(`.js,`).join(`, `).slice(0, -3);
            const config = fs.readdirSync(`src/commands/config`).toString().split(`.js,`).join(`, `).slice(0, -3);
            const img = fs.readdirSync(`src/commands/img`).toString().split(`.js,`).join(`, `).slice(0, -3);
